...
public class RAGTest {
    ...
    public void upload() throws IOException  {
        // 获取项目根目录的绝对路径
        TextReader reader = new TextReader("data/file.txt");

        List<Document> documents = reader.get();
        List<Document> documentSplitterList = tokenTextSplitter.apply(documents);

        documents.forEach(doc -> doc.getMetadata().put("knowledge", "知识库名称"));
        documents.forEach(doc -> doc.getMetadata().put("charset", "UTF-8"));
        documentSplitterList.forEach(doc -> doc.getMetadata().put("knowledge", "知识库名称"));
        documentSplitterList.forEach(doc -> doc.getMetadata().put("charset", "UTF-8"));
